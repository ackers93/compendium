...
class CsvCrossReferenceImportsController < ApplicationController
  ...
  def create
    @csv_xref_text = params[:csv_text].to_s
    source = csv_source
    ...
    result = Imports::CsvCrossReferencesImporter.call(user: current_user, io_or_string: source)
    @xref_import_result = result
  ...
  def import_notice(result)
    ...
    if result.imported_count.positive?
      parts << "Imported #{result.imported_count} cross-#{'reference'.pluralize(result.imported_count)}"
    end
    parts.concat(skip_summary_parts(result.skipped))
    return "No cross-references were imported." if parts.empty?
  ...
  def import_alert(result)
    parts = []
    parts << "#{result.imported_count} cross-#{'reference'.pluralize(result.imported_count)} imported" if result.imported_count.positive?
